...
class Simulation_arrays(object):
    ...
    def __init__(self, n_fishers, delta, q, r, K, R_0, e_0, price, cost, noise, num_feedback, payoff_discount, num_steps):
        ...
        self.dR = np.zeros(n_fishers)
        self.e_new = np.zeros(n_fishers)
        self.e_data = np.zeros((n_fishers, num_steps))
        self.R_data = np.zeros((n_fishers, num_steps))
        self.U_data = np.zeros((n_fishers, num_steps))
